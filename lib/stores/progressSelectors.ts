...
// as a parameter rather than reading the curriculum itself, so it stays pure
// and the caller decides which account's tree is being aggregated.
export function computeStats(
  ...
  groups.forEach((group) => {
    group.sections.forEach((section) => {
      ...
      const completed = Object.keys(store).filter(
        (k) => k.startsWith(prefix) && store[k],
      ).length;
...
export function computeSectionStats(
  ...
  const done = Object.keys(store).filter(
    (k) => k.startsWith(prefix) && store[k],
  ).length;
  return { done, total };
}
