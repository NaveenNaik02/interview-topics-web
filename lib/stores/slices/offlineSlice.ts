...
> = (set, get) => ({
  ...
  enableOfflineMode: async () => {
    const { user, store, totals, groups } = get();
    if (!user) return;
    set({ isCaching: true });
    ...
    // Snapshot current progress and question totals to localStorage. Priority
    // and starred are cached per-section below (they're columns on the
    // questions row now, fetched in the same query as title/bodyHtml).
    ...
    for (const section of allSections) {
      try {
        const { data } = await supabase
          .from('questions')
          .select('id, number, title, body_html, starred, priority')
  ...
  disableOfflineMode: async () => {
    ...
    if (
      isOnline &&
      user &&
      (getPendingOps().length > 0 || getPendingPriorityOps().length > 0)
    ) {
      set({ isSyncing: true });
  ...
  syncNow: async () => {
    ...
    if (okProgress && okPriority) {
      ...
      set({
        pendingOpsCount:
          getPendingOps().length + getPendingPriorityOps().length,
      });
    }
    set({ isSyncing: false });
  },
