...
export function getOfflineEnabled(): boolean {
  try {
    return localStorage.getItem(OFFLINE_KEYS.OFFLINE_ENABLED) === 'true';
  } catch {
    return false;
  }
}
export function setOfflineEnabled(val: boolean): void {
  try {
    localStorage.setItem(OFFLINE_KEYS.OFFLINE_ENABLED, String(val));
  } catch {}
}
...
export function setCachedProgress(ids: string[]): void {
  try {
    localStorage.setItem(OFFLINE_KEYS.PROGRESS_CACHE, JSON.stringify(ids));
  } catch {}
}
...
export function getCachedAt(): string | null {
  try {
    return localStorage.getItem(OFFLINE_KEYS.CACHED_AT);
  } catch {
    return null;
  }
}
export function setCachedAt(ts: string): void {
  try {
    localStorage.setItem(OFFLINE_KEYS.CACHED_AT, ts);
  } catch {}
}
...
export function setCachedTotals(totals: Record<string, number>): void {
  try {
    localStorage.setItem(OFFLINE_KEYS.QUESTION_TOTALS, JSON.stringify(totals));
  } catch {}
}
...
export function setCachedQuestions(
  ...
  try {
    localStorage.setItem(sectionKey(topic, file), JSON.stringify(questions));
  } catch {}
}
...
function setPendingOps(ops: PendingOp[]): void {
  try {
    localStorage.setItem(OFFLINE_KEYS.PENDING_OPS, JSON.stringify(ops));
  } catch {}
}
...
export function clearPendingOps(): void {
  try {
    localStorage.removeItem(OFFLINE_KEYS.PENDING_OPS);
  } catch {}
}
...
function setPendingPriorityOps(ops: PriorityPendingOp[]): void {
  try {
    localStorage.setItem(
      OFFLINE_KEYS.PENDING_PRIORITY_OPS,
      JSON.stringify(ops),
    );
  } catch {}
}
...
export function clearPendingPriorityOps(): void {
  try {
    localStorage.removeItem(OFFLINE_KEYS.PENDING_PRIORITY_OPS);
  } catch {}
}
...
export function getCachedUserId(): string | null {
  try {
    return localStorage.getItem(OFFLINE_KEYS.USER_ID);
  } catch {
    return null;
  }
}
export function setCachedUserId(id: string): void {
  try {
    localStorage.setItem(OFFLINE_KEYS.USER_ID, id);
  } catch {}
}
...
export async function flushPendingPriorityOps(): Promise<boolean> {
  ...
  try {
    await Promise.all([
      upserts.length > 0 ? questionFlagsActions.bulkSetPriority(upserts) : null,
      removes.length > 0
        ? questionFlagsActions.bulkClearPriority(removes)
        : null,
    ]);
