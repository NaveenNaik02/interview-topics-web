...
export function TopicsProvider({ groups, children }: { groups: TopicGroup[]; children: React.ReactNode }) {
  return <TopicsContext.Provider value={groups}>{children}</TopicsContext.Provider>
}

export function useTopicGroups(): TopicGroup[] {
  const groups = useContext(TopicsContext)
  if (!groups) throw new Error('useTopicGroups must be used within a TopicsProvider')
  return groups
}
