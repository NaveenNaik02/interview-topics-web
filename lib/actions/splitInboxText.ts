...
export async function splitInboxText(text: string, model?: string): Promise<string[]> {
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) throw new Error('Not authenticated')
  if (user.is_anonymous) throw new Error('Sign in to use AI split')
  ...
  const chosenModel = AQ_MODELS.some((m) => m.id === model) ? (model as AqModelId) : DEFAULT_MODEL
  ...
  if (!res.ok) {
    throw new Error(body?.error?.message || 'Could not split that — try again.')
  }
  ...
  if (!Array.isArray(parsed)) throw new Error('Could not read the split result — try again.')

  const questions = parsed.map((s) => String(s).trim()).filter(Boolean)
  if (questions.length === 0) throw new Error('Could not find any questions in that text.')
  return questions
}
