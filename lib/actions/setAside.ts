...
export async function setAsideQuestion(id: string): Promise<SetAsideItem> {
  ...
  const { data: existing } = await supabase
    .from('questions')
    .select('title, markdown, body_html, lang, tags, problem, topic, file, label')
    .eq('id', id)
    .maybeSingle()
  ...
  const { data: inserted, error: insertError } = await supabase
    ...
    .single()
  if (insertError || !inserted) throw insertError ?? new Error('Could not set aside — try again.')
...
export async function discardSetAsideItem(id: string): Promise<void> {
  const { supabase, user } = await requireUser()
  const { error } = await supabase.from('set_aside_items').delete().eq('id', id).eq('user_id', user.id)
