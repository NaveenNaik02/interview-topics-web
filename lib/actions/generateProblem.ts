...
export async function generateProblem(input: GenerateProblemInput): Promise<string> {
  ...
  if (user.is_anonymous && !isLocalSupabase()) throw new Error('Sign in to generate a problem statement')
  ...
  if (!res.ok) {
    throw new Error(body?.error?.message || 'Could not generate a problem statement — try again.')
  }

  const text = body?.candidates?.[0]?.content?.parts?.[0]?.text
  if (!text) throw new Error('Could not generate a problem statement — try again.')
  return text.trim()
}
