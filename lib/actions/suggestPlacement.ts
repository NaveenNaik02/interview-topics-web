...
export type PlacementSuggestion =
  | { mode: 'existing'; groupSlug: string; topic: string; file: string; reasoning: string }
  | { mode: 'new-subtopic'; groupSlug: string; label: string; reasoning: string }
  | { mode: 'new-topic'; topicName: string; blurb: string; label: string; reasoning: string }
...
function buildTree(groups: SuggestPlacementGroup[]): string {
  return groups
    .map((g) =>
      ...
      g.sections.map((s) => `  - ${s.label} (topic: "${s.topic}", file: "${s.file}")`).join('\n')
    )
    .join('\n')
}

export async function suggestPlacement(input: SuggestPlacementInput): Promise<PlacementSuggestion> {
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) throw new Error('Not authenticated')
  if (user.is_anonymous) throw new Error('Sign in to use AI placement')
  ...
  if (!res.ok) {
    throw new Error(body?.error?.message || 'Could not get a placement suggestion — try again.')
  }

  const text = body?.candidates?.[0]?.content?.parts?.[0]?.text
  if (!text) throw new Error('Could not get a placement suggestion — try again.')
...
function validate(parsed: unknown, groups: SuggestPlacementGroup[]): PlacementSuggestion {
  const bad = () => new Error('Could not get a placement suggestion — try again.')
