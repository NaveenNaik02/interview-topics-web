...
There is no auto-provisioned session in either environment — dev and prod both require a real login at `/login`. Locally that's the seeded `dev@local.test` account (password in `.env.local`'s `DEV_PW`), typed into the email/password form like any other account. See "Auth & admin roles" below.
...
`lib/content/` is the curriculum layer: `topics.ts` (types + pure helpers + the static `TOPIC_GROUPS` seed), `topicsData.ts` and `parser.ts` (both `server-only`). **No barrel** — most of `topics.ts`'s 35 consumers are client components, and a barrel would drag the two `server-only` siblings into their bundles.

- `lib/content/parser.ts` (`server-only`) queries Supabase. `countQuestions`/`parseSection` hit the `questions` table, populated by `scripts/etl.js` (which renders HTML via `remark` at ETL time, not query time) plus any questions authored directly in the app (HTML rendered via `marked` + `isomorphic-dompurify` at write time instead — see below).
...
- **Anonymous auth is gone.** Nothing in the app calls `signInAnonymously` anymore, and `lib/supabase/middleware.ts` gates every route on `!!user && !user.is_anonymous`, redirecting to `/login` otherwise (only `/login`, `/signup`, `/auth/*`, `/manifest.json`, `/sw.js` are exempt). Sign-in is email/password, Google, or GitHub — all in `features/login/actions/auth.ts`, surfaced by `app/login/page.tsx` + `app/signup/page.tsx`; OAuth returns through `/auth/callback`, which calls `exchangeCode`. `authSlice` only holds `signInWithGitHub`/`signOut` plus the `onAuthStateChange` subscription. The gate is identical in dev and prod, so authoring actions need no per-environment branching — but existing anonymous rows may still sit in `auth.users` from before the switch.
...
- Authoring server actions (add/edit/delete question, add topic/section, AI generation) call `requireAuthor()` (`lib/supabase/user.ts`), which throws unless the caller is signed in and not anonymous. Since the middleware already enforces that on every route, the check is now defence-in-depth rather than the thing that separates two live user classes.
...
`scripts/pull-remote.js` (`npm run pull-remote`) seeds **local** Docker Supabase from the cloud project: it copies content + the source account's study data and rewrites every `created_by`/`user_id` to `DEV_USER`, so local dev sees a real dataset. It derives `sections` rows from the imported questions (broader than `backfill-static-sections.js`, which only knows the static list). Cloud has since caught up on `20260804105557_merge_starred_priority_into_questions`, so `starred`/`priority` now arrive as columns on `questions`; the script's `legacy()` fallback for the old `starred_questions`/`priority` tables is dead weight kept only in case it's pointed at an older project. Upsert-only and safe to re-run; it never writes to the remote.

### Dynamic topics & authoring

- `lib/content/topics.ts` holds the static `TOPIC_GROUPS` curriculum plus shared types (`TopicGroup`/`SectionMeta`) and pure helpers (`findGroup`, `sectionUrl`, `slugify`, etc.), which take a `groups` array as an explicit parameter. **`TOPIC_GROUPS` is no longer read at runtime** — its only consumer is `scripts/backfill-static-sections.js`, which uses it as seed data.
- New tables `topic_groups` and `sections` (migrations `20260715120000_topic_groups.sql`, `20260716120000_topic_delete.sql`) let signed-in users add topics/subtopics at runtime.
- `lib/content/topicsData.ts`'s `getAllGroups()` (request-memoized via React `cache()`, server-only) returns **only the caller's own** `topic_groups` + `sections` rows — no merge with `TOPIC_GROUPS`, so it's an empty array for a brand-new account. Every reader of the curriculum (routing, sidebar, dashboard, Add Question pickers) goes through this.
...
- **AI-assisted authoring** — everything Gemini-facing lives in `lib/ai/` (`generateQuestion.ts`, `generateAnswer.ts`, `generateBlurb.ts`, `generateProblem.ts`, `generateCodeOutput.ts`, `suggestPlacement.ts`, `checkDuplicate.ts`, `formatAnswer.ts`), leaving `lib/actions/` as the plain data mutations. These all call Google Gemini's REST API directly (no SDK) using `FREE_GEM_API_KEY`, gated behind sign-in + non-anonymous. `lib/ai/models.ts` exports the shared model list (`gemini-3.1-flash-lite` default, `gemini-3-flash-preview`). Each action is an independent `'use server'` function that re-derives auth itself (`requireAuthor`), builds its prompt from `lib/ai/prompts.ts`, and makes the actual call through `lib/ai/gemini.ts` — `gemini()` for plain text, `geminiJson()` when the prompt asks for JSON (it adds `responseMimeType` and strips code fences before parsing). The wrapper owns the API key, model validation, and error handling; it deliberately does *not* do auth, since the message is per-action and `checkDuplicate` needs the `supabase` client `requireAuthor` returns. `suggestPlacement` and `checkDuplicate` re-validate the model's JSON response against the real curriculum/question list server-side since the model can hallucinate ids.
- `components/AddQuestionFab.tsx` (draggable via `lib/hooks/useFabDrag.ts`) opens `AddTopicModal` or the large `AddQuestionModal.tsx` (markdown preview via `marked` + `isomorphic-dompurify`, wires up all AI actions, keeps an in-memory-only `AnswerVersion[]` history of generated drafts — this is the "answer versioning" from commit `fca05e1`; it's a UI convenience, not a DB-level version table).
...
- **Syntax highlighting** — `components/QuestionItem/highlight.ts` wraps Prism.js, highlighting the `<pre><code class="language-xxx">` DOM that `marked` already produces (no re-render needed).
...
- **Hooks** — a hook used by one feature lives in that feature's `hooks/` folder; one shared across features lives in `lib/hooks/` (barrel: `@/lib/hooks`). Nothing hook-shaped stays loose at `lib/` root.
- **FAB drag** — `lib/hooks/useFabDrag.ts` uses its own small Zustand store (`useFabOffsetStore`, separate from `useAppStore` — pure client UI concern, and the FABs render outside `UIProvider` in `layout.tsx`) so dragging any one FAB moves the whole bottom-right cluster together; resets on route change.
