...
export function useAnswerVersions(
  ...
  const [activeVersionId, setActiveVersionId] = useState<string | null>(() =>
    originalMarkdown.trim() ? 'original' : null,
  );
    ...
    setAnswerVersions((prev) => {
      ...
      const label =
        activeVersionId === 'original'
          ? 'Original'
          : prev.find((v) => v.id === activeVersionId)?.label;
      if (label && prev.some((v) => v.text === markdown)) return prev;
      draftCountRef.current += 1;
      return capAnswerVersions([
        ...prev,
        {
          id: `v${Date.now()}`,
          label: label ?? `Draft ${draftCountRef.current}`,
          text: markdown,
        },
      ]);
    });
  ...
  const addAnswerVersion = (text: string) => {
    ...
    setAnswerVersions((prev) =>
      capAnswerVersions([
        ...prev,
        { id, label: `Draft ${draftCountRef.current}`, text },
      ]),
    );
