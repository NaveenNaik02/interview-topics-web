...
//
// Only a count is kept here (for the Sidebar badge) — full item bodies live
// page-side, fetched by the Inbox page itself via fetchInitialInboxPageData
// and kept fresh via revalidatePath('/inbox') in the mutating actions.
export const createInboxSlice: StateCreator<AppState, [], [], InboxSlice> = (
  ...
  loadInboxCount: async (uid: string) => {
    const count = await inboxDb.fetchInboxCount(uid);
